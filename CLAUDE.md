...
### 進捗報告のグラウンディング
- 進捗・完了の報告は必ず今セッションのツール結果で裏付ける
- 未検証項目は「未検証」と明示。テスト失敗は出力付きでそのまま報告
- 証拠なき「できました」禁止

### ターン終了前チェック（途中停止防止）
- ターン終了前に最終段落を確認: 計画・分析・「これから〜します」宣言で終わるの禁止
- 宣言した作業は今すぐツール実行。エラーは自分でリトライ
- 停止条件: タスク完了 or ユーザーにしか答えられない入力待ちのみ

### 実装中の自律性（Planモード承認後に適用）
- 承認済み計画の範囲内なら、可逆的操作は確認せず実行
- 些細な選択（命名・デフォルト値・同等手法の選択）は自分で決めて注記
- 確認必須: 破壊的操作・スコープ変更のみ（既存ルールどおり）

### 結論先行
- 最初の1文=結果（何が起きたか/何が見つかったか）。詳細は後
- 長時間作業後の報告は「初見の読者向け」に書く。作業中の略語・造語持ち込み禁止
